...
Although the senior portfolios project, leveraging Google Sites, has existing functionality for comments, portfolio reviewers and student authors need a way to share private comments only visible to those within the engineering honors program. Moreover, reviewers need a way to sift through the large number of comments flowing in from all of the portfolios. Thus, the EHP Portfolios Private Comments application allows authorized EHP community members and students to share portfolio feedback privately in a tailored portfolio-specific interface.
...
All non-handler code requires at least 80% code coverage via unit tests. Additionally, all server-side code must be commented using the "Epydoc":http://epydoc.sourceforge.net/ format.
